...
public class Config {
	...
	public SecurityFilterChain configuration(HttpSecurity http) throws Exception {
		...
		}).authorizeHttpRequests(auth -> auth.requestMatchers(HttpMethod.POST, "/customers/", "/admin/").permitAll()
				.requestMatchers("/swagger-ui*/**", "/v3/api-docs/**").permitAll()
				.requestMatchers("/customers/**").hasRole("CUSTOMER")
				.requestMatchers("/operator/**").hasRole("OPERATOR")
				.requestMatchers("/admin/**").hasRole("ADMIN")
//				.requestMatchers("customers/signin").hasRole("CUSTOMER")
//				.requestMatchers("admin/signin").hasRole("ADMIN")
//				.requestMatchers("operator/signin").hasRole("OPERATOR")
